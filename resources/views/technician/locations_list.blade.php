<x-technician-layout hideHeader="true">
    <div class="fixed top-0 inset-x-0 z-[60] bg-[#0a0d14]/95 backdrop-blur-3xl border-b border-white/5 pt-safe">
        <div class="px-4 py-4 flex items-center justify-between mx-auto max-w-2xl">
            <a href="{{ route('technician.infrastructure') }}" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition shadow-lg">
    ...
    <div class="pt-24 px-5 max-w-2xl mx-auto md:py-10 md:px-0">
        <div class="space-y-3">
            @foreach($locations as $loc)
                <div class="bg-gradient-to-r from-[#12161f] to-[#0a0d14] border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:border-tecsisa-yellow/20 transition active:scale-95 shadow-lg group">
                    <div class="flex items-center gap-4 min-w-0 flex-1">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-sm font-black text-white group-hover:text-purple-400 transition truncate">{{ $loc->name }}</h4>
                    ...
                    <div class="flex gap-2">
                        <button class="bg-white/5 border border-white/10 p-2 rounded-xl text-gray-400 hover:text-white transition">
