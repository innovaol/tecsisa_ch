<x-technician-layout hideHeader="true">
    ...
    <div class="fixed top-0 inset-x-0 z-[60] bg-[#0a0d14]/95 backdrop-blur-3xl border-b border-white/5 pt-safe">
        <div class="px-4 py-4 flex items-center justify-between mx-auto max-w-lg">
            <button onclick="window.history.back()" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition shadow-lg">
    ...
    <div class="pt-24 px-5 max-w-lg mx-auto" x-data="scannerApp()">
        ...
        <div x-show="activeTab === 'scan'" x-transition class="flex flex-col items-center">
            ...
            <button @click="toggleCamera()" class="mt-8 px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition shadow-2xl flex items-center gap-3" :class="isScanning ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-tecsisa-yellow text-black'">
                <span x-text="isScanning ? 'Detener Pantalla' : 'Volver a Intentar'"></span>
                <svg x-show="!isScanning" class="w-5 h-5 font-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path></svg>
            </button>
        </div>

        <!-- MANUAL SEARCH VIEW -->
        <div x-show="activeTab === 'text'" x-transition style="display: none;" class="flex flex-col">
            <div class="bg-gradient-to-br from-[#12161f] to-[#0a0d14] rounded-[2.5rem] border border-white/10 p-8 shadow-2xl relative overflow-hidden text-center mb-10">
                <div class="w-20 h-20 bg-white/5 border border-white/5 rounded-[2rem] flex items-center justify-center mx-auto mb-6">
                    <svg class="w-10 h-10 text-tecsisa-yellow" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                </div>
                <h2 class="text-xl font-black text-white uppercase tracking-widest mb-3">Carga Manual</h2>
